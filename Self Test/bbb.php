<?php
    ...
    // 파일정보들 확인.($file은 5칸 짜리 배열)
    $file_name=$file['name']; //원본 파일명
    $file_size=$file['size']; //파일 사이즈(byte)
    $file_type=$file['type']; //파일 타입(image/jap, audio/mps -> MIME type)
    ...
    // $temp_name 위치에 있는 파일의 실제데이터는 임시공간이기에 이 코드가 종료되면 사라짐
    // 이 코드가 종료되면 사라짐
    // 그래서 반드시 서버에서 할당된 내 저장소(html폴더 내부)안으로 이동해야 함
    // 이동시킬 곳의 파일명이 중복되면 안되기에 보통은 날짜정보를 파일명으로 사용함

    $dst_name = "./uploaded/".date("YmdHis").$file_name; //php에서는 문자열의 결합연산자가 .
