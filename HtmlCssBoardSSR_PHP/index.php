<?php
    ...
    //백엔드 코드를 화면 html 코드와 함께 작성하는 것이 가능함
    //데이터베이스의 web_board 테이블에 게시글 리스트를 읽어와서 아래 html에 표시

    //MySQL DRMS와 연결하여 데이터들 가져오기
    $db = mysqli_connect('localhost', 'axvax56', 'a1s2d3f4!', 'axvax56'); //DB주소, DB접속아이다, DB접속비번, DB명
    mysqli_query($db, 'set names utf8'); //한글 깨짐 방지

    //원하는 쿼리문 수행. SQL언어 - web_board 테이블의 모든 데이터들 번호기준 내림차순으로(게시판은 최신글 순으로)
    $sql = "SELECT * FROM web_board ORDER BY no DESC";
    $result = mysqli_query($db, $sql);

    //요청한 결과표($result)로부터 게시글들을 한줄씩 가져와서 $board_list 라는 이름의 배열에 추가하기
    $board_list = []; //빈 배열 준비
    // DB에서 데이터를 가져오는 것은 한번에 한줄씩 가져와야 함 -> 반복문 필요
    ...
    // 총 게시글 수를 알아내기
    $board_size = count($board_list); //count: 파이썬의 len()과 같은 기능
    ...
    <div class="board_wrap">
        ...
        <div class="board_list_wrap">
            ...
            <div class="board_pagination">
                <a href="" class="btn">&lt;&lt;</a>
                <a href="" class="btn">&lt;</a>
                <a href="" class="btn selected">1</a> <!-- btn 과 selected 라는 2개의 클래스명을 사용-->
